...
export const handlePrimaryUuid = (knex: Knex, table: Knex.CreateTableBuilder) => (
    column?: string
) =>
    table
        .uuid(column || 'id')
...
export const handleForeignUuid = (table: Knex.CreateTableBuilder) => (
    column: string,
...
export const schema = (knex: Knex) => {
    return function columns(table: Knex.CreateTableBuilder) {
        return {
            ...
            deleted_at: table.dateTime('deleted_at').defaultTo(null)
        }
    }
}
